...
BRAIN_DIR = ROOT_DIR / "AI_Guidance" / "Brain"
...
def sync_brain_to_quint(dry_run: bool = False, verbose: bool = False) -> dict:
    ...
    core_context_dir = ROOT_DIR / "AI_Guidance" / "Core_Context"
    if core_context_dir.exists():
        context_files = sorted(core_context_dir.glob("*-context.md"), reverse=True)
        if context_files:
    ...
    # Add reasoning state
    context_sections.append("\n## Reasoning State\n")
    context_sections.append(f"- DRRs: {len(list((REASONING_DIR / 'Decisions').glob('*.md')))}")
    context_sections.append(f"- Active cycles: {len(list((QUINT_DIR / 'sessions').glob('*')))}")
...
"""
    ...
    if verbose:
        print(f"  Updated .quint/context.md with {results['entities_imported']} entities")
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
        help="Show detailed sync log"
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="Show reasoning state summary"
    )
    ...
    if not any([args.to_brain, args.to_quint, args.bidirectional]):
        print("No sync direction specified. Use --to-brain, --to-quint, or --bidirectional")
        return
