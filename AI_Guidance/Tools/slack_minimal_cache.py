...
OUTPUT_DIR = Path("AI_Guidance/Brain/Inbox/Slack/Cache")
AUDIT_FILE = Path("AI_Guidance/Brain/Inbox/Slack/cache_audit.json")
...
def fetch_user_info(client, user_id: str) -> dict:
    ...
    try:
        ...
        if e.response["error"] == "user_not_found":
            return {"id": user_id, "name": f"Unknown ({user_id})", "error": "user_not_found"}
            ...
            return {"id": user_id, "name": f"Error ({user_id})", "error": str(e.response["error"])}
...
def fetch_channel_info(client, channel_id: str) -> dict:
    ...
    try:
        ...
        if e.response["error"] in ["channel_not_found", "method_not_supported_for_channel_type"]:
            return {"id": channel_id, "name": f"Unknown ({channel_id})", "error": e.response["error"]}
        elif e.response["error"] == "ratelimited":
            import time
            retry_after = int(e.response.headers.get("Retry-After", 5))
            print(f"  Rate limited, waiting {retry_after}s...", file=sys.stderr)
            ...
            return {"id": channel_id, "name": f"Error ({channel_id})", "error": str(e.response["error"])}
...
def build_minimal_cache(audit: dict) -> tuple:
    ...
    for i, user_id in enumerate(user_ids, 1):
        ...
        print(f"  [{i}/{len(user_ids)}] {user_id} → {user_info.get('name', 'Unknown')}", file=sys.stderr)
    ...
    for i, channel_id in enumerate(channel_ids, 1):
        ...
        print(f"  [{i}/{len(channel_ids)}] {channel_id} → #{channel_info.get('name', 'Unknown')}", file=sys.stderr)
...
def main():
    ...
    # Load audit
    audit = load_audit()
    print(f"Audit: {audit['total_users']} users, {audit['total_channels']} channels needed", file=sys.stderr)
