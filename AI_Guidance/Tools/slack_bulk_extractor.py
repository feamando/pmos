...
BASE_DIR = Path("AI_Guidance/Brain/Inbox/Slack")
...
CHANNEL_TIERS = {
    "tier1": {
        ...
        "channels": [
            {"id": "C06CT0185T3", "name": "tribe-new-ventures-leads-internal"},
            {"id": "C06CE7FB951", "name": "tribe-new-ventures-internal"},
            {"id": "C08KU0F8EF4", "name": "tribe-new-ventures-product"},
            {"id": "C03DDD4R537", "name": "squad-nv-good-chop"},
    ...
    "tier2": {
        ...
        "channels": [
            {"id": "C02RQHFTJ0V", "name": "good-chop-product"},
            {"id": "C025A4WK3J4", "name": "goodchop"},
            {"id": "C04DT1T9RMG", "name": "tpt_team"},
            {"id": "C07TGTQSZDQ", "name": "consumer-mega-alliance"},
...
def print_status(state: dict):
    ...
    if in_progress:
        ...
        for channel_id, info in in_progress.items():
            print(f"  - {info.get('name', channel_id)}: Week {info.get('current_week', '?')}")
    ...
    while True:
        try:
            ...
            if e.response["error"] == "ratelimited":
                retry_after = int(e.response.headers.get("Retry-After", RATE_LIMIT_BACKOFF))
...
def save_raw_messages(channel_id: str, channel_name: str, week_label: str, messages: list):
...
def extract_channel(
    ...
    for week_start, week_end, week_label in weeks:
        ...
        if messages:
            output_file = save_raw_messages(channel_id, channel_name, week_label, messages)
            print(f"{len(messages)} messages, {thread_count} threads → {output_file.name}", file=sys.stderr)
    ...
    print(f"\nChannel complete: {total_messages} messages, {total_threads} threads", file=sys.stderr)
...
def run_extraction(
    ...
    print("=" * 60, file=sys.stderr)
    print(f"Period: {start_date.date()} to {end_date.date()} ({days} days)", file=sys.stderr)
    ...
    if resume:
        ...
        print(f"Remaining: {len(channels)} (after filtering completed)", file=sys.stderr)
    ...
    print("=" * 60, file=sys.stderr)
    print(f"This run: {grand_total_messages:,} messages, {grand_total_threads:,} threads", file=sys.stderr)
    print(f"Total (all runs): {state.get('total_messages', 0):,} messages", file=sys.stderr)
    print(f"Channels completed: {len(state.get('channels_completed', []))}", file=sys.stderr)
    print("=" * 60, file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from last extraction state"
    )
    parser.add_argument(
        "--status",
        action="store_true",
