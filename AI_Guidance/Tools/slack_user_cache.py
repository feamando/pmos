...
DEFAULT_OUTPUT_DIR = "AI_Guidance/Brain/Inbox/Slack"
...
def fetch_all_users(client, output_dir: str = None, save_every: int = 50) -> dict:
    ...
    if output_dir:
        ...
        if progress_file.exists():
            with open(progress_file, "r", encoding="utf-8") as f:
                ...
                print(f"Resuming from page {page} with {len(users)} users cached", file=sys.stderr)
    ...
    while True:
        try:
            ...
            print(f"  Page {page}: {len(response.get('members', []))} users", file=sys.stderr)
...
def _save_progress(output_dir: str, cache_type: str, data: dict, cursor: str, page: int):
    ...
    print(f"  [Saved progress: {len(data)} {cache_type}s at page {page}]", file=sys.stderr)


def fetch_all_channels(client, include_private=True, output_dir: str = None, save_every: int = 100) -> dict:
    """
    ...
    if output_dir:
        ...
        if progress_file.exists():
            with open(progress_file, "r", encoding="utf-8") as f:
                ...
                cursor = progress.get("cursor")
                page = progress.get("page", 0)
                print(f"Resuming from page {page} with {len(channels)} channels cached", file=sys.stderr)
    ...
    while True:
        try:
            ...
            print(f"  Page {page}: {len(response.get('channels', []))} channels", file=sys.stderr)
...
def load_user_cache(output_dir: str = DEFAULT_OUTPUT_DIR) -> dict:
    ...
    if not filepath.exists():
        raise FileNotFoundError(f"User cache not found at {filepath}. Run slack_user_cache.py first.")
...
def load_channel_cache(output_dir: str = DEFAULT_OUTPUT_DIR) -> dict:
    ...
    if not filepath.exists():
        raise FileNotFoundError(f"Channel cache not found at {filepath}. Run slack_user_cache.py first.")
    with open(filepath, "r", encoding="utf-8") as f:
        return json.load(f)
...
def resolve_user(user_id: str, cache: dict) -> str:
    ...
    if user_id in cache:
        data = cache[user_id]
        return data.get("display_name") or data.get("name") or data.get("username") or user_id
    return user_id
