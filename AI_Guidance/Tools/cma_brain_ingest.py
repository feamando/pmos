...
# Configuration
INBOX_FILE = "AI_Guidance/Brain/Inbox/INBOX_2025-12-08-.md"
JIRA_TOOL = "python AI_Guidance/Tools/jira_mcp/server.py --cli search_issues"
ENTITIES_DIR = "AI_Guidance/Brain/Entities"
REGISTRY_FILE = "AI_Guidance/Brain/registry.yaml"
...
def parse_inbox_file():
    ...
    # Find the CMA Squad doc section
    start_marker = "### DOC: CMA Squad overview"
    end_marker = "----------------------------------------"
    
    start_idx = content.find(start_marker)
    if start_idx == -1:
        print("DEBUG: Could not find start marker '### DOC: CMA Squad overview'")
        return []
    ...
    if csv_start == -1:
         print(f"DEBUG: Could not find header marker '{header_marker}' after index {start_idx}")
         return []
...
def main():
    ...
    for row in reader:
        ...
        if 'projects/' in backlog_url:
            try:
                # Extract key (e.g. projects/GOC/boards/...)
...
"""
        ...
        # Register Squad
        new_registry_entries[squad] = {'type': 'Team', 'aliases': [f"{squad} Squad"], 'file': f"Entities/{squad_filename}"}
        ...
        for person, role in [(pm, "Product Manager"), (em, "Engineering Manager")]:
            if person and len(person) > 2: # Basic check
                ...
                # Register Person
                new_registry_entries[person] = {'type': 'Person', 'file': f"Entities/{person_filename}"}
