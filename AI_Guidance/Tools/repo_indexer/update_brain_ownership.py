...
def parse_squad_map(map_path):
    ...
    for line in lines:
        ...
        squads = [s.strip().replace('@hellofresh/', '') for s in squads_str.split(',')]
...
def find_entity_file(squad_name, entities_dir):
    ...
    # Normalize squad name for search (e.g., squad-good-chop -> good chop)
...
def update_brain_entities():
    base_dir = Path(__file__).parents[3] / 'AI_Guidance' / 'Brain'
