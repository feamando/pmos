...
- Filters by topics (New Ventures, Good Chop, TPT, Factor Form, etc.)
...
BRAIN_DIR = Path("AI_Guidance/Brain")
...
def run_processor(dry_run: bool = False, resume: bool = True, list_docs: bool = False):
    ...
    if list_docs:
        ...
        print("=" * 60)
        for doc in sorted(filtered_docs, key=lambda x: x.get("filter_score", 0), reverse=True):
    ...
    if dry_run:
        print(f"\n[DRY RUN] Would create batches for {len(filtered_docs)} documents", file=sys.stderr)
        return
    ...
    for batch in batches:
        ...
        print(f"  Saved: {filepath.name} ({batch['doc_count']} docs, {batch['total_chars']:,} chars)", file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        "--no-resume",
        action="store_true",
        help="Reprocess all inbox files"
    )
    parser.add_argument(
        "--list-docs",
        action="store_true",
        help="List all matched documents and exit"
    )
    parser.add_argument(
        "--status",
        action="store_true",
