...
Examples:
    python3 file_chunker.py --check AI_Guidance/Brain/Inbox/INBOX_2025-12-05.md
    python3 file_chunker.py --split AI_Guidance/Brain/Inbox/INBOX_2025-12-05.md
    python3 file_chunker.py --scan AI_Guidance/Brain/Inbox --threshold 1500
"""
...
# State tracking
STATE_DIR = Path("AI_Guidance/Tools/chunker_state")
...
def analyze_file(file_path: str, max_lines: int = DEFAULT_MAX_LINES,
                 max_bytes: int = DEFAULT_MAX_BYTES) -> FileInfo:
...
def scan_directory(dir_path: str, max_lines: int = DEFAULT_MAX_LINES,
                   pattern: str = "*.md") -> List[FileInfo]:
...
def split_file(file_path: str, max_lines: int = DEFAULT_MAX_LINES,
...
def write_chunks(chunks: List[Chunk], source_path: str,
...
def print_file_info(info: FileInfo):
    ...
    if info.needs_chunking:
        print(f"  Suggested: {info.suggested_chunks} chunks of ~{info.chunk_size} lines each")
...
def print_chunking_result(result: ChunkingResult):
    ...
    for i, chunk in enumerate(result.chunks):
        print(f"  [{i+1}] Lines {chunk.start_line+1}-{chunk.end_line} ({chunk.lines} lines)")
...
def main():
    ...
    parser.add_argument('--check', type=str, metavar='FILE',
                        help='Check if a file needs chunking')
    ...
    parser.add_argument('--status', action='store_true',
                        help='Show chunking status')

    # Options
    parser.add_argument('--output', '-o', type=str,
                        help='Output directory for chunks')
    parser.add_argument('--threshold', '-t', type=int, default=DEFAULT_MAX_LINES,
        ...
        if args.json:
            ...
            if status['tracked_files']:
                ...
                for tf in status['tracked_files']:
                    print(f"  {tf['source']} → {tf['chunks']} chunks ({tf['timestamp'][:10]})")
        return
    ...
    if args.split:
        try:
            ...
            if args.dry_run:
                ...
                for chunk in chunks:
                    print(f"  Chunk {chunk.index}: lines {chunk.start_line+1}-{chunk.end_line} ({chunk.lines} lines)")
                return
