...
Usage:
    ...
    python slack_context_poster.py <context_file> --channel C0A6ZAS1MSQ
...
DEFAULT_CHANNEL = "C0A6ZAS1MSQ"  # #ngo-slack-private


def get_slack_client() -> WebClient:
    """Initialize Slack client from environment."""
...
def parse_context_file(file_path: str) -> dict:
    ...
    if critical_match:
        alerts = re.findall(r'^- \*\*(.+?)\*\*:?\s*(.*)$', critical_match.group(1), re.MULTILINE)
        result['critical_alerts'] = [{'title': a[0], 'detail': a[1].strip()} for a in alerts]
    ...
    if action_match:
        ...
        items = re.findall(r'^- \[ \] \*\*(.+?)\*\*:?\s*(.*)$', action_match.group(1), re.MULTILINE)
    ...
    if blocker_match:
        rows = re.findall(r'^\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|', blocker_match.group(1), re.MULTILINE)
        result['blockers'] = [{'blocker': r[0], 'impact': r[1], 'owner': r[2]} for r in rows]
    ...
    if dates_match:
        rows = re.findall(r'^\|\s*\*?\*?(.+?)\*?\*?\s*\|\s*(.+?)\s*\|', dates_match.group(1), re.MULTILINE)
        result['key_dates'] = [{'date': r[0].strip('*'), 'milestone': r[1]} for r in rows]
...
def format_slack_message(parsed: dict, msg_type: str = "update") -> list:
    ...
    emoji_map = {"boot": ":rocket:", "update": ":arrows_counterclockwise:", "logout": ":checkered_flag:"}
    title_map = {
        "boot": "Context Loaded",
    ...
    if msg_type == "logout" and parsed['session_summary']:
        ...
        blocks.append({
            "type": "section",
            "text": {"type": "mrkdwn", "text": f"*Session Summary:*\n{summary_preview}"}
        })
    ...
    blocks.append({
        "type": "context",
        "elements": [
            {"type": "mrkdwn", "text": f"Posted {datetime.now().strftime('%Y-%m-%d %H:%M')} CET"}
        ]
    })
...
def post_to_slack(client: WebClient, channel: str, blocks: list, fallback_text: str) -> dict:
...
def main():
    ...
    parser.add_argument('--channel', default=DEFAULT_CHANNEL, help=f'Slack channel ID (default: {DEFAULT_CHANNEL})')
    parser.add_argument('--type', choices=['boot', 'update', 'logout'], default='update',
                       help='Message type (affects header/formatting)')
    parser.add_argument('--test', action='store_true', help='Test Slack connection only')
    parser.add_argument('--dry-run', action='store_true', help='Parse and format but do not post')
    ...
    if args.test:
        try:
            ...
            print(f"OK: Channel access confirmed: #{info['channel'].get('name', args.channel)}")
            return 0
