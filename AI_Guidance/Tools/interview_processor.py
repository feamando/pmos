...
"""
Interview Processor
Analyzes interview transcripts against HelloFresh DNA and Career Framework.
...
INBOX_DIR = ROOT_DIR / "AI_Guidance" / "Brain" / "Inbox"
OUTPUT_DIR = ROOT_DIR / "Team" / "Interviews" / "Notes"
FRAMEWORKS_DIR = ROOT_DIR / "AI_Guidance" / "Frameworks"
...
def get_drive_service():
    ...
    if not creds or not creds.valid:
        ...
        if not creds or not creds.valid:
            print("Warning: Valid GDrive token not found. Skipping GDrive fetch.", file=sys.stderr)
            return None
...
def load_frameworks():
    ...
    frameworks = ""
    for fname in ["HelloFresh_DNA.md", "Product_Career_Framework.md"]:
...
def analyze_interview(title, transcript, frameworks):
    ...
    prompt = f"""You are an expert Hiring Manager Assistant at HelloFresh.
...
2. **DNA Assessment** (Analyze fit against HelloFresh DNA values: Speed, Data, Egolessness, etc. Provide evidence).
