...
def upload_files_to_store(store_name: str, files: List[Path], verbose: bool = True) -> Dict[str, str]:
    ...
    for i, file_path in enumerate(files):
        if verbose:
            rel_path = str(file_path.relative_to(ROOT_DIR)) if file_path.is_relative_to(ROOT_DIR) else file_path.name
        ...
        try:
            ...
            doc_name = result.response.document_name if result.response else str(result.name)
...
def list_store_documents(store_name: str) -> List[Dict]:
    ...
    try:
        result = client.file_search_stores.documents.list(file_search_store_name=store_name)
        for doc in result:
            docs.append({
                "name": doc.name,
...
def main():
    ...
    parser.add_argument('--create', action='store_true', help='Create new store and index all files')
    parser.add_argument('--sync', action='store_true', help='Sync new files to existing store')
    parser.add_argument('--list', action='store_true', help='List all File Search stores')
    parser.add_argument('--list-docs', action='store_true', help='List documents in configured store')
    parser.add_argument('--delete', action='store_true', help='Delete the configured store')
