...
Usage:
    python3 prd_generator.py --topic "Add OTP to Good Chop"
    python3 prd_generator.py --update Products/Good_Chop/OTP_PRD.md --instructions "Add competitor analysis"
...
def poll_interaction(client, interaction_id: str, max_time: int = 600, interval: int = 10) -> Dict:
    ...
    while True:
        ...
        try:
            ...
            print(f"  Researching... ({minutes}m {seconds}s) - Status: {status}", end='\r')
...
def generate_prd_with_deep_research(topic: str, store_name: Optional[str] = None) -> str:
    ...
    # Add file search if store configured
    if store_name:
        tools_config.append(
            types.Tool(
    ...
    # Add Google Search for web research
    tools_config.append(
        types.Tool(
            google_search=types.GoogleSearch()
        )
    ...
    try:
        ...
        if "429" in error_msg or "quota" in error_msg.lower() or "rate" in error_msg.lower():
            print(f"\nDeep Research quota exceeded. Falling back to standard generation...", file=sys.stderr)
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate PRDs using Deep Research'
    )
    parser.add_argument('--topic', type=str, help='Topic for new PRD')
    parser.add_argument('--update', type=str, metavar='FILE', help='Path to existing PRD to update')
    parser.add_argument('--instructions', type=str, default='', help='Update instructions (with --update)')
    parser.add_argument('--output', type=str, help='Output file path')
    parser.add_argument('--setup', action='store_true', help='Run initial setup (index files)')
    parser.add_argument('--no-deep-research', action='store_true', help='Use standard generation instead of Deep Research')
    ...
    if args.update:
        ...
        instructions = args.instructions or "Review and improve the PRD with latest information"
