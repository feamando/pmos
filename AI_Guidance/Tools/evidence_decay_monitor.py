...
class EvidenceItem:
    ...
    def __init__(self, file_path: Path, expiry_date: datetime,
                 description: str = "", drr_ref: str = ""):
...
def generate_report(items: List[EvidenceItem], days: int) -> str:
    ...
    if expired:
        ...
        lines.append("|----------|---------|------|--------|")
        for item in expired:
            rel_path = item.file_path.relative_to(REPO_ROOT) if REPO_ROOT in item.file_path.parents else item.file_path
            lines.append(f"| {item.description} | {item.expiry_date.strftime('%Y-%m-%d')} | `{rel_path}` | **Refresh immediately** |")
        lines.append("")
    ...
    if critical:
        ...
        for item in critical:
            rel_path = item.file_path.relative_to(REPO_ROOT) if REPO_ROOT in item.file_path.parents else item.file_path
            lines.append(f"| {item.description} | {item.expiry_date.strftime('%Y-%m-%d')} | {item.days_until_expiry} | `{rel_path}` |")
        lines.append("")
...
def print_summary(items: List[EvidenceItem], days: int):
    ...
    for item in filtered:
        ...
        days_text = f"{item.days_until_expiry}d" if item.days_until_expiry >= 0 else "EXPIRED"
        print(f"{icon} [{item.status:8}] {item.description[:40]:40} | {days_text:8} | {item.expiry_date.strftime('%Y-%m-%d')}")
...
def send_slack_notification(items: List[EvidenceItem], days: int) -> bool:
    ...
    if expired_count + critical_count > 0:
        ...
        items_text = "\n".join([f"• {i.description[:50]} ({i.status})" for i in urgent_items])
...
def main():
    parser = argparse.ArgumentParser(
        description="Monitor FPF evidence for expiration"
    )
    parser.add_argument("--days", type=int, default=DEFAULT_WARNING_DAYS,
