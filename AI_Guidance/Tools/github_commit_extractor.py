...
OUTPUT_DIR = ROOT_DIR / "AI_Guidance" / "Brain" / "Inbox" / "GitHub"
    ...
    try:
        ...
        if result.returncode != 0:
            if "404" not in result.stderr:
                print(f"  Warning: gh api error: {result.stderr[:100]}", file=sys.stderr)
            return None
...
def fetch_commits(repo: str, since_days: int = DEFAULT_LOOKBACK_DAYS,
                  prefixes: List[str] = None) -> List[Dict]:
    """
    Fetch commits from a repo.

    Args:
        repo: Full repo name (e.g., "hellofresh/web")
        since_days: How far back to look
        prefixes: Optional list of prefixes to filter by (e.g., ["GOC-", "TPT-"])
    ...
    while len(commits) < MAX_COMMITS_PER_REPO:
        endpoint = f"repos/{repo}/commits?since={since_date}&per_page={per_page}&page={page}"
...
def extract_all_commits(since_days: int = DEFAULT_LOOKBACK_DAYS) -> Dict[str, List[Dict]]:
    """
...
def create_batches(all_commits: Dict[str, List[Dict]]) -> List[Dict]:
    ...
    for i in range(0, len(flat_commits), BATCH_SIZE):
        ...
        batches.append({
            ...
            "date_range": {
                "oldest": batch_commits[-1].get("date", "") if batch_commits else "",
...
def save_state(all_commits: Dict[str, List[Dict]], batches: List[Dict]):
    ...
    state = {
        ...
        "commits_per_repo": {repo: len(commits) for repo, commits in all_commits.items()},
    }
...
def main():
    ...
    if args.dry_run:
        ...
        for batch in batches[:5]:
            print(f"    - {batch['batch_id']}: {batch['commit_count']} commits", file=sys.stderr)
        if len(batches) > 5:
