...
Fetches GitHub activity (PRs, commits) for New Ventures squads and writes to Brain
...
Usage:
    ...
    python3 github_brain_sync.py --squad "Good Chop"  # Single squad
...
BRAIN_PROJECTS_DIR = ROOT_DIR / "AI_Guidance" / "Brain" / "Projects"
...
def fetch_recent_commits(repo: str, pr_prefix: str, days: int = DEFAULT_LOOKBACK_DAYS) -> List[Dict]:
    ...
    for commit in commits:
        ...
        if pr_prefix and (pr_prefix in first_line or pr_prefix.lower() in first_line.lower()):
...
def find_prs_for_jira_ticket(ticket_key: str, repos: List[str] = None) -> List[Dict]:
    ...
    if repos is None:
        repos = ['hellofresh/web']  # Default to web repo
...
def fetch_squad_github(squad: Dict) -> Dict[str, Any]:
    ...
    if not github_repos:
        ...
        github_repos = [{
            'repo': 'hellofresh/web',
            'pr_prefix': f'{jira_project}-'
        }]

    try:
        for repo_config in github_repos:
            repo = repo_config.get('repo', 'hellofresh/web')
...
def fetch_all_squads_parallel(squads: List[Dict]) -> Dict[str, Dict]:
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        futures = {
            executor.submit(fetch_squad_github, squad): squad
            for squad in squads
        }
    ...
    for squad_name, squad_data in data.items():
        ...
        if prs:
            ...
            for pr in prs:
                ...
                lines.append(f"| [#{pr['number']}]({pr['url']}) | {pr['title'][:50]}{draft_marker} | @{pr['author']} | {pr['age']} | {pr['state']} |")
        else:
        ...
        if commits:
            for commit in commits[:10]:  # Limit to 10 in output
                lines.append(f"- `{commit['sha']}` {commit['message'][:60]} (@{commit['author']}, {commit['age']})")
        else:
...
def write_pr_activity_file(data: Dict[str, Dict]) -> Path:
    ...
    lines = [
        "# PR Activity - New Ventures Squads",
        "",
...
def write_recent_commits_file(data: Dict[str, Dict]) -> Path:
    ...
    lines = [
        "# Recent Commits - New Ventures Squads",
        "",
        f"*Auto-updated: {timestamp}*",
...
def update_squad_entity(squad_name: str, squad_data: Dict) -> bool:
    ...
    filename_map = {
        'Good Chop': 'Squad_Good_Chop.md',
        'The Pets Table': 'Squad_The_Pets_Table.md',
        'Factor Form': 'Squad_Factor_Form.md',
        'Market Innovation': 'Squad_Market_Innovation.md',
    }
...
"""
        ...
        if re.search(github_status_pattern, content, re.DOTALL):
            content = re.sub(github_status_pattern, new_section.rstrip(), content, flags=re.DOTALL)
        else:
            ...
            if changelog_match:
                ...
                jira_end = re.search(r'## Jira Status\n.*?(?=\n## |\Z)', content, re.DOTALL)
                if jira_end:
...
def summarize_with_gemini(data: Dict[str, Dict]) -> Optional[str]:
    ...
    for squad_name, squad_data in data.items():
        ...
        squad_summary += f"Recent Commits: {len(squad_data.get('recent_commits', []))}\n"

        if squad_data.get('open_prs'):
            squad_summary += "Open PRs:\n"
            for pr in squad_data['open_prs'][:5]:
                squad_summary += f"- #{pr['number']} {pr['title'][:50]} (Age: {pr['age']})\n"

        summary_input.append(squad_summary)

    prompt = f"""Analyze this GitHub activity for New Ventures squads and provide a brief summary (3-5 bullet points) highlighting:
...
def main():
    ...
    parser.add_argument(
        '--squad',
        type=str,
        help='Sync specific squad only (e.g., "Good Chop")'
    )
    parser.add_argument(
        '--summarize',
        action='store_true',
        help='Include Gemini-generated summary'
    )
    parser.add_argument(
        '--analyze-files',
        action='store_true',
    ...
    parser.add_argument(
        '--output',
        type=str,
        help='Custom output path for inbox file'
    )
    ...
    if args.analyze_files:
        ...
        for squad_name, squad_data in data.items():
            ...
            for pr in squad_data.get('open_prs', [])[:5]:  # Limit to top 5 PRs
                repo = pr.get('repo', 'hellofresh/web')
