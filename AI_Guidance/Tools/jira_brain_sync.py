...
Fetches recent Jira updates from New Ventures squads and writes to Brain
...
Usage:
    ...
    python3 jira_brain_sync.py --squad "Good Chop"  # Single squad
...
def fetch_squad_jira(squad: Dict, jira: Jira) -> Dict[str, Any]:
    ...
    try:
        ...
        in_progress_jql = f'project = {project_key} AND status = "In Progress" ORDER BY updated DESC'
...
def fetch_all_squads_parallel(squads: List[Dict]) -> Dict[str, Dict]:
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        futures = {
            executor.submit(fetch_squad_jira, squad, jira): squad
            for squad in squads
        }
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"  ✓ {squad['name']}: {epic_count} epics, {ip_count} in-progress, {blocker_count} blockers")
            except Exception as e:
...
def write_inbox_file(data: Dict[str, Dict], output_path: Optional[Path] = None) -> Path:
    ...
    for squad_name, squad_data in data.items():
        ...
        if epics:
            ...
            lines.append("|-----|---------|--------|----------|")
            for epic in epics:
                lines.append(f"| {epic['key']} | {epic['summary'][:60]} | {epic['status']} | {epic['assignee']} |")
        else:
        ...
        if blockers:
            for item in blockers:
                priority_str = f"(Priority: {item['priority']})" if item['priority'] != 'None' else ''
                lines.append(f"- **[{item['key']}]** {item['summary'][:80]} {priority_str}")
...
"""
        ...
        if re.search(jira_status_pattern, content, re.DOTALL):
            content = re.sub(jira_status_pattern, new_section.rstrip(), content, flags=re.DOTALL)
        else:
...
def summarize_with_gemini(data: Dict[str, Dict]) -> Optional[str]:
    ...
    for squad_name, squad_data in data.items():
        ...
        if squad_data.get('blockers'):
            ...
            for b in squad_data['blockers'][:5]:
                squad_summary += f"- [{b['key']}] {b['summary']} (Priority: {b['priority']})\n"
    ...
    prompt = f"""Analyze this Jira status for New Ventures squads and provide a brief executive summary (3-5 bullet points) highlighting:
    ...
    for repo in repos:
        try:
            ...
            result = subprocess.run(
                [gh_path, 'api', f'search/issues?q=repo:{repo}+is:pr+{issue_key}+in:title&per_page=5'],
...
def main():
    parser = argparse.ArgumentParser(
        description='Sync Jira data to Brain for New Ventures squads'
    )
    parser.add_argument(
        '--squad',
        type=str,
    ...
    parser.add_argument(
        '--github',
        action='store_true',
        help='Include GitHub PR/commit links (Phase 2)'
    )
    parser.add_argument(
        '--no-entities',
        action='store_true',
