...
BRAIN_DIR = Path("AI_Guidance/Brain/Inbox")
...
MODEL_ID = os.getenv("ANTHROPIC_MODEL", "eu.anthropic.claude-sonnet-4-5-20250929-v1:0")
...
def analyze_slack_batch(batch: Dict, client, dry_run: bool = False) -> Dict:
    ...
    if dry_run:
        print(f"  [DRY RUN] Would analyze {len(messages)} messages from #{channel_name}", file=sys.stderr)
        return {
            "batch_id": batch.get("batch_id"),
    ...
    print(f"  Analyzing {len(messages)} messages from #{channel_name}...", file=sys.stderr)
...
def run_analysis(source: str, batch_file: Optional[str] = None, analyze_all: bool = False,
    ...
    for batch_path in batches:
        ...
        if not dry_run:
            ...
            state["docs_analyzed"] += analysis.get("doc_count", analysis.get("message_count", analysis.get("commit_count", analysis.get("issue_count", 0))))
    ...
    # Summary
    print(f"\n{'=' * 60}", file=sys.stderr)
    print(f"{source.upper()} ANALYSIS {'COMPLETE' if not dry_run else 'DRY RUN COMPLETE'}", file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        "--batch",
        help="Single batch file to analyze"
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Analyze all unprocessed batches"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
