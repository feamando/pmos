...
def get_confluence_client():
    ...
    try:
        ...
        print("atlassian-python-api not installed. Run: pip install atlassian-python-api", file=sys.stderr)
        return None

    if not CONFLUENCE_URL or not CONFLUENCE_EMAIL or not CONFLUENCE_API_TOKEN:
        print("Confluence credentials not configured. Set CONFLUENCE_URL, CONFLUENCE_EMAIL, CONFLUENCE_API_TOKEN", file=sys.stderr)
        return None
...
def sync_search_results(query: str, space_key: Optional[str] = None, limit: int = 10) -> List[Path]:
    """
...
def main():
    ...
    parser.add_argument(
        "--search",
        type=str,
        help="Search Confluence for pages matching query"
    )
    parser.add_argument(
        "--page",
        type=str,
        help="Fetch and sync a specific page by ID"
    )
    parser.add_argument(
        "--space",
        type=str,
        help="Confluence space key to search/sync"
    )
    parser.add_argument(
        "--recent",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Maximum pages to return (default: 10)"
    )
    parser.add_argument(
        "--sync",
        action="store_true",
        help="Sync search results to Brain"
    )
    parser.add_argument(
        "--list-synced",
        action="store_true",
        help="List pages already synced to Brain"
    )
    parser.add_argument(
        "--json",
        action="store_true",
