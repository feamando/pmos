...
CLAUDE_MODEL_ID = os.getenv("CLAUDE_MODEL_ID", "eu.anthropic.claude-sonnet-4-5-20250929-v1:0")
...
def invoke_claude(
    ...
    try:
        ...
        return {
            "error": f"Failed to connect to AWS Bedrock: {e}",
            "model": "claude"
        }
...
def _invoke_gemini_standard(
    ...
    try:
        ...
        return {
            "error": f"Gemini invocation failed: {e}",
            "model": "gemini"
        }
...
def _invoke_gemini_deep_research(prompt: str, api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "error": f"Gemini Deep Research failed: {e}",
            "model": "gemini"
        }
...
def main():
    ...
    parser.add_argument(
        "--context",
        type=str,
        help="Path to context file (markdown or JSON)"
    )
    parser.add_argument(
        "--detect",
        action="store_true",
        help="Detect active model and exit"
    )
    parser.add_argument(
        "--deep-research",
        action="store_true",
