...
def get_parser():
    ...
    if CACHE_AVAILABLE:
        try:
            ...
            print("Warning: Cache files not found. Run slack_user_cache.py first.", file=sys.stderr)
            return MrkdwnParser()
    return None
    ...
    try:
        ...
        while True:
            ...
            for user in response["members"]:
                ...
                if search_name in real_name or search_name in display_name or search_name == name:
                    return user["id"]
...
def extract_messages(
    ...
    if not channel_id:
        if not channel_name:
            print("Error: Must provide either --channel or --channel_id", file=sys.stderr)
            return
    ...
    if parser and channel_id in parser.channel_cache:
        resolved_channel_name = parser.channel_cache[channel_id].get("name", channel_name)

    print(f"Extracting messages from #{resolved_channel_name} ({channel_id})...", file=sys.stderr)
    ...
    try:
        ...
        print(f"Found {len(all_messages)} messages in the last {days} days.", file=sys.stderr)
        ...
        for msg in all_messages:
            ...
            if format_for_brain and parser:
                ...
                ts_str = datetime.fromtimestamp(float(msg_ts)).strftime('%Y-%m-%d %H:%M:%S')
                ...
                if reactions:
                    reaction_str = " ".join([f":{r['name']}:({r.get('count', 1)})" for r in reactions])
            ...
            if include_threads and msg.get("reply_count", 0) > 0:
                ...
                for reply in replies:
                    ...
                    if format_for_brain and parser:
                        ...
                        ts_str = datetime.fromtimestamp(float(reply_ts)).strftime('%Y-%m-%d %H:%M:%S')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--list-channels", action="store_true", help="List all channels the bot is in")
    parser.add_argument(
        "--threads",
        action="store_true",
        default=True,
