...
- Supports New Ventures tribe squads (Good Chop, TPT, Factor Form, MIO)
- Orthogonal challenge mode for Claude<>Gemini review
- Multiple output formats (Markdown, Google Docs compatible)

Usage:
    python3 tribe_quarterly_update.py --tribe "New Ventures"
    python3 tribe_quarterly_update.py --tribe "New Ventures" --orthogonal
    python3 tribe_quarterly_update.py --tribe "New Ventures" --squad "Good Chop"
...
BRAIN_DIR = REPO_ROOT / "AI_Guidance" / "Brain"
CONTEXT_DIR = REPO_ROOT / "AI_Guidance" / "Core_Context"
PLANNING_DIR = REPO_ROOT / "Planning"
OUTPUT_DIR = PLANNING_DIR / "Quarterly_Updates"
TEMPLATE_DIR = PLANNING_DIR / "Templates"
SQUAD_REGISTRY_PATH = REPO_ROOT / "squad_registry.yaml"

# Tribe configuration
TRIBES = {
    "New Ventures": {
        "mega_alliance": "Consumer Mega-Alliance",
        "mission": "Incubate, validate, and scale new business ventures beyond HelloFresh's core meal kit offering.",
        "squads": ["Good Chop", "The Pets Table", "Factor Form", "Market Innovation"],
        "jira_projects": ["GOC", "TPT", "RTEVMS", "MIO"],
        "lead": "Nikita Gorshkov",
        "eng_lead": "Daniel Arias",
    }
}
...
def gather_brain_projects(tribe: str) -> List[Dict[str, Any]]:
    ...
    search_patterns.extend([
        "otp", "vms", "factor_form", "good_chop", "tpt", "pet", "mio",
        "marketplace", "shopify", "portion", "beam"
    ])
...
def extract_key_initiatives(context_files: List[Dict], projects: List[Dict]) -> List[Dict]:
    ...
    for ctx in context_files[:5]:
        ...
        for pattern, metric_name in patterns:
            ...
            for match in matches:
                ...
                if key not in seen_metrics and metric_name not in [m.split(':')[0].strip('- *') for m in metrics]:
...
def generate_systemic_blockers(blockers: List[str], jira_data: Dict) -> str:
    ...
    for blocker in blockers:
        ...
        if '|' in blocker:
            # Parse table rows: "Good Chop 492 orders delayed | Customer experience | Ops/Laurence"
            parts = [p.strip() for p in blocker.split('|') if p.strip()]
            if len(parts) >= 2 and not any(h in parts[0] for h in ['Blocker', 'Impact', 'Owner', '---']):
    ...
    all_blockers = clean_blockers[:3] if clean_blockers else [
...
def generate_roadmap_table(initiatives: List[Dict], tribe_config: Dict) -> str:
    ...
    # Pad to 5 rows
    while len(rows) < 5:
        rows.append("| [Strategy] | [Project] | [Description] | [KPI] | [DoD] | [Dependencies] |")
...
def generate_tribe_update(
    ...
    document = TRIBE_UPDATE_TEMPLATE.format(
        ...
        mega_alliance=tribe_config.get("mega_alliance", "Consumer Mega-Alliance"),
        mission=tribe_config.get("mission", "[Tribe mission]"),
        executive_metrics=executive_metrics,
        strategic_relevance="New Ventures drives diversification revenue streams and de-risks HelloFresh's dependency on the core meal kit business. The tribe's success directly impacts the company's long-term growth trajectory and TAM expansion.",
...
def save_document(content: str, tribe: str, quarter: str = CURRENT_QUARTER, output_path: Optional[Path] = None) -> Path:
...
def main():
    parser = argparse.ArgumentParser(
        description="Tribe Quarterly Update Generator"
    )
    parser.add_argument(
        "--tribe",
        type=str,
        default="New Ventures",
        choices=list(TRIBES.keys()),
        help="Tribe name (default: New Ventures)"
    )
    parser.add_argument(
        "--quarter",
        type=str,
    ...
    parser.add_argument(
        "--status",
        action="store_true",
        help="Show status of existing updates"
    )
    parser.add_argument(
        "--json",
        action="store_true",
