...
Usage:
    python3 orthogonal_challenge.py --type prd --topic "Push notifications for TPT"
...
from research_aggregator import gather_context, format_for_prompt
...
def run_round_2(
    ...
    round2_fpf = {
        ...
        "critical_count": sum(
            1 for c in challenges.get("challenges", [])
...
def run_round_3(challenge_id: str) -> Dict[str, Any]:
    ...
    with open(v3_path, 'w', encoding='utf-8') as f:
        f.write(f"# {DOC_TYPES.get(state.get('doc_type', ''), 'Document')}: {state.get('topic', 'Unknown')}\n\n")
...
def main():
    ...
    parser.add_argument(
        "--status",
        action="store_true",
        help="List all challenges and their status"
    )
    parser.add_argument(
        "--resume",
        type=str,
