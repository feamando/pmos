...
# Add parent directory to path to import common.config_loader
sys.path.append(str(Path(__file__).parent.parent))
from common import config_loader

# Constants
BRAIN_DIR = config_loader.get_root_path() / "AI_Guidance" / "Brain"
...
def parse_args():
    ...
    parser.add_argument("--deep", action="store_true", help="Sync all experiments (including finished)")
    parser.add_argument("--dry-run", action="store_true", help="Fetch data but do not write files")
    parser.add_argument("--summary-file", help="Write a markdown summary of processed experiments to this file")
...
class StatsigSync:
    ...
    def generate_markdown(self, item, item_type="experiment"):
        ...
        if created_time > 10000000000: created_time = created_time / 1000
...
def main():
    ...
    syncer = StatsigSync()
    syncer.sync(active_only=args.active_only, dry_run=args.dry_run, summary_file=args.summary_file)
