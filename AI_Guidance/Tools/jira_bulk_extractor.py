...
OUTPUT_DIR = ROOT_DIR / "AI_Guidance" / "Brain" / "Inbox" / "Jira"
...
TOPIC_FILTERS = [
    "new ventures", "good chop", "goodchop", "goc",
    "the pets table", "pets table", "tpt",
    "factor form", "vms", "market innovation", "market integration",
    "cross-selling", "cross selling", "crossselling",
    "otp", "one time purchase", "one-time purchase",
    "seasonal boxes", "seasonal box", "occasion boxes", "occasion box",
    "shopify", "reactivation", "dashboard", "checkout",
]
...
def get_jira_client() -> Jira:
    ...
    if not conf['url'] or not conf['username'] or not conf['api_token']:
        raise ValueError("Jira configuration missing in .env (JIRA_URL, JIRA_USERNAME, JIRA_API_TOKEN)")
...
def load_squad_registry(filter_tribe: Optional[str] = "New Ventures") -> List[Dict]:
    ...
    if not SQUAD_REGISTRY_PATH.exists():
        print(f"Error: Squad registry not found at {SQUAD_REGISTRY_PATH}", file=sys.stderr)
        return []
    ...
    for link in fields.get('issuelinks', []):
        if link.get('outwardIssue'):
            links.append({
                ...
                'summary': link['outwardIssue'].get('fields', {}).get('summary', '')[:100],
            })
        elif link.get('inwardIssue'):
            links.append({
                'type': link.get('type', {}).get('inward', 'relates to'),
                'key': link['inwardIssue'].get('key'),
                'summary': link['inwardIssue'].get('fields', {}).get('summary', '')[:100],
            })

    return {
        'key': issue.get('key'),
        'summary': fields.get('summary', ''),
        'description': (fields.get('description') or '')[:2000],  # Truncate long descriptions
        ...
        'sprint': extract_sprint_name(fields.get('customfield_10020', [])),  # Common sprint field
        'comments': comments,
...
def fetch_project_issues(project_key: str, jira: Jira, since_days: int = DEFAULT_LOOKBACK_DAYS) -> List[Dict]:
    """
    Fetch issues from a Jira project.

    Args:
        project_key: Jira project key (e.g., "GOC", "TPT")
        jira: Jira client
...
def fetch_all_projects_parallel(projects: List[Dict], since_days: int) -> Dict[str, List[Dict]]:
...
def create_batches(all_issues: Dict[str, Dict]) -> List[Dict]:
    ...
    for i in range(0, len(flat_issues), BATCH_SIZE):
        ...
        batches.append({
            ...
            "issues": batch_issues,
            "date_range": {
                "oldest": batch_issues[-1].get("updated", "") if batch_issues else "",
                "newest": batch_issues[0].get("updated", "") if batch_issues else "",
            }
        })
...
def save_state(all_issues: Dict[str, Dict], batches: List[Dict]):
    ...
    state = {
        ...
        "issues_per_project": {k: len(v.get('issues', [])) for k, v in all_issues.items()},
    }
...
def show_status():
    ...
    # Check batches
    batch_files = list(PROCESSED_DIR.glob("batch_*.json")) if PROCESSED_DIR.exists() else []
    ...
    if state_file.exists():
        with open(state_file, "r", encoding="utf-8") as f:
            state = json.load(f)
        print(f"Last extraction: {state.get('extracted_at', 'Unknown')}", file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        "--status",
        action="store_true",
        help="Show extraction status and exit"
    )
    parser.add_argument(
        "--project",
        type=str,
        help="Extract specific project only (e.g., GOC, TPT)"
    )
    ...
    if args.dry_run:
        ...
        for batch in batches[:5]:
            print(f"    - {batch['batch_id']}: {batch['issue_count']} issues ({batch['issue_types']})", file=sys.stderr)
        if len(batches) > 5:
    ...
    print(f"  Batches: {len(batches)}", file=sys.stderr)
    print("=" * 60, file=sys.stderr)
    print("\nNext step: python3 batch_llm_analyzer.py --source jira --all", file=sys.stderr)
