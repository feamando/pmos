...
Usage:
    python3 tech_context_sync.py --analyze hellofresh/web
...
# Constants
TECHNICAL_BRAIN_DIR = ROOT_DIR / "AI_Guidance" / "Brain" / "Technical"
...
SPEC_MACHINE_REPO = "hellofresh/spec-machine"
...
PRIORITY_REPOS = [
    {"name": "hellofresh/web", "type": "frontend", "description": "Consumer web frontend"},
    {"name": "hellofresh/whitelabel-mobile", "type": "mobile", "description": "React Native mobile apps"},
    {"name": "hellofresh/engagement-dau", "type": "backend", "description": "Engagement services"},
    {"name": "hellofresh/consumer-bff", "type": "bff", "description": "Consumer BFF layer"},
]
...
def fetch_file_content(repo: str, path: str, ref: Optional[str] = None) -> Optional[str]:
...
def detect_tech_stack(repo: str) -> Dict[str, Any]:
    ...
    if pkg_content:
        try:
            ...
            # Key dependencies
            key_deps = ["@apollo/client", "graphql", "zod", "react-hook-form",
...
def sync_spec_machine_standards() -> Dict[str, List[str]]:
    ...
    for category, files in categories.items():
        ...
        for filepath in files[:5]:  # Limit to 5 files per category
            content = fetch_file_content(SPEC_MACHINE_REPO, filepath, ref=SPEC_MACHINE_BRANCH)
            if content:
...
*Source: hellofresh/spec-machine/profiles/engagement-dau/standards/{category}/*
...
def main():
    ...
    parser.add_argument("--sync-spec-machine", action="store_true",
                       help="Sync standards from spec-machine")
    parser.add_argument("--list-repos", action="store_true",
                       help="List priority repos for analysis")
    parser.add_argument("--all", action="store_true",
                       help="Analyze all priority repos")
