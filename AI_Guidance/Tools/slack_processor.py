...
try:
    ...
    from slack_user_cache import load_user_cache, load_channel_cache
...
HIGH_VALUE_KEYWORDS = {
    "decision": ["decided", "decision", "agreed", "approved", "confirmed", "finalized", "go ahead", "green light"],
    "blocker": ["blocker", "blocked", "blocking", "stuck", "waiting on", "dependency", "can't proceed"],
    "question": ["?", "anyone know", "does anyone", "how do we", "what's the", "when will", "who is"],
    "action": ["action item", "todo", "to do", "will do", "I'll", "let me", "can you", "please"],
    "deadline": ["deadline", "due date", "by EOD", "by EOW", "by friday", "urgent", "asap"],
    "update": ["update:", "fyi:", "heads up", "just wanted to", "quick update", "status:"],
    "announcement": ["announcing", "announcement", "excited to share", "please welcome", "congrats"],
}
...
def process_raw_file(filepath: Path, parser: Optional[object] = None) -> list:
    ...
    for msg in messages:
        ...
        processed_msg = {
            "ts": msg.get("ts"),
            "user": msg.get("user"),
            "user_name": parser.resolve_user(msg.get("user", "")) if parser else msg.get("user"),
            "text": parsed_text,
            "raw_text": raw_text,
            ...
            for reply in msg["_thread_replies"]:
                if not is_noise_message(reply):
                    ...
                    thread_replies.append({
                        "ts": reply.get("ts"),
                        "user": reply.get("user"),
                        "user_name": parser.resolve_user(reply.get("user", "")) if parser else reply.get("user"),
                        "text": parsed_reply,
                    })
...
def run_processing(batch_size: int = DEFAULT_BATCH_SIZE, resume: bool = True):
    ...
    if PARSER_AVAILABLE:
        try:
            ...
            print("Warning: Cache files not found, parsing without resolution", file=sys.stderr)
            parser = MrkdwnParser()
    ...
    for filepath in raw_files:
        ...
        state["total_messages_in"] = state.get("total_messages_in", 0) + len(messages) + filtered
    ...
    for i, batch in enumerate(batches):
        if batch:
            ...
            print(f"  Saved: {filepath.name} ({batch['message_count']} messages)", file=sys.stderr)
