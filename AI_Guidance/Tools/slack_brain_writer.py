...
def write_decision(decision: dict, channel_name: str, dry_run: bool = False) -> Optional[Path]:
    """
...
def process_analysis_file(
    ...
    for entity in analysis.get("entities", []):
        ...
        if name:
            ...
            success = update_entity_context(name, etype, context_entry, aliases, state, dry_run)
            if success:
                stats["entities"] += 1
...
def run_brain_writer(dry_run: bool = False, resume: bool = True):
    ...
    for filepath in files:
        ...
        if not dry_run:
            ...
            state["decisions_logged"] = state.get("decisions_logged", 0) + stats["decisions"]
...
def main():
    parser = argparse.ArgumentParser(
        description="Write analyzed Slack data to Brain"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--no-resume",
        action="store_true",
        help="Reprocess all analysis files"
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="Show brain writer status and exit"
    )
