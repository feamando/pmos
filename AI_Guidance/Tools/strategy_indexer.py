...
RAW_DIR = os.path.join(BASE_DIR, '../Brain/Inbox/Strategy_Raw')
...
def extract_titles_from_entities():
    ...
    for fname in os.listdir(ENTITIES_DIR):
        if not fname.startswith('Domain_'): continue
        ...
        matches = re.findall(r'\[(Domain Objective Document|Yearly Plan Document)\]\((.*?)\)', content)
