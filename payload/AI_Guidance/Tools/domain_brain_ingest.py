...
ENTITIES_DIR = os.path.join(BASE_DIR, '../Brain/Entities')
STRATEGY_DIR = os.path.join(BASE_DIR, '../Brain/Strategy')
REGISTRY_FILE = os.path.join(BASE_DIR, '../Brain/registry.yaml')
...
def safe_filename(name):
    return clean_name(name).replace(' ', '_').replace('&', 'and').replace('/', '_').replace(':', '').replace('\n', '_')
...
def process_dos_tab(ws, registry_updates):
    ...
    for i, row in enumerate(ws.iter_rows(values_only=True)):
        ...
        if 'domain' in row_str and ('driver' in row_str or 'owner' in row_str):
            ...
            for col_idx, cell in enumerate(row):
                ...
                if c_val == 'domain': headers['domain'] = col_idx
                if 'central functions' in c_val: headers['alliance'] = col_idx
                if 'driver' in c_val: headers['comm_leader'] = col_idx
                if 'yp owner' in c_val or '2026-yp owner' in c_val: headers['tech_leader'] = col_idx
                ...
                if 'refined' in c_val and 'document' in c_val: headers['do_doc'] = col_idx
                if 'yp final' in c_val or ('final' in c_val and 'market' in c_val): headers['yp_doc'] = col_idx
            break
    ...
    for row in ws.iter_rows(min_row=header_row_idx+1, values_only=True):
        ...
        if isinstance(domain_val, (int, float)) or (isinstance(domain_val, str) and domain_val.replace('.','').isdigit()):
            # Check next column
        ...
        if alliance_val:
            ...
            # Check if ID
            if isinstance(alliance_val, (int, float)) or (isinstance(alliance_val, str) and alliance_val.replace('.','').isdigit()):
                 # Check next column
        ...
        if domain_name.lower() == 'domain': continue # header repeat?
        if domain_name == 'None': continue 
        
        # Extract other fields
        comm_leader = clean_name(row[headers['comm_leader']]) if 'comm_leader' in headers else ""
        tech_leader = clean_name(row[headers['tech_leader']]) if 'tech_leader' in headers else ""
                ...
                if len(p) > 2 and 'TBD' not in p and 'done' not in p.lower():
                    ...
                    if not os.path.exists(p_path):
                        with open(p_path, 'w', encoding='utf-8') as f:
                            f.write(f"# {p}\n\n## Metadata\n- **Type**: Person\n- **Role**: Leadership\n")
                    registry_updates[p] = {'type': 'Person', 'file': f"Entities/{p_fname}"}
