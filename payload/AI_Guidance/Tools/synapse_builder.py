...
Usage:
    python synapse_builder.py           # Run on all files
...
def normalize_link(link: str) -> str:
    ...
    # Remove [[ and ]] and "AI_Guidance/Brain/" prefix if present
    clean = link.replace('[[', '').replace(']]', '')
    # We want relative path from Brain/ root, e.g. "Entities/Nikita.md"
    if clean.startswith('AI_Guidance/Brain/'):
        clean = clean.replace('AI_Guidance/Brain/', '')
    return clean
...
def main(dry_run: bool = False):
    ...
    for target_file, needed_rels in updates_needed.items():
        ...
        for rel_type, sources in needed_rels.items():
            ...
            current_values = set(normalize_link(x) for x in fm['relationships'][rel_type])
