...
def find_last_message_by_user(client: WebClient, user_id: str, limit: int = 100) -> dict:
    """
        ...
        for channel in conversations['channels']:
            try:
                ...
                if e.response['error'] not in ['channel_not_found', 'not_in_channel']:
                    print(f"  Warning: Could not read {channel.get('name', channel['id'])}: {e.response['error']}")
                continue
...
def main():
    ...
    if user_info:
        print(f"   Found: {user_info.get('real_name', 'Unknown')} (@{user_info.get('name', 'unknown')})")
    else:
    ...
    if last_msg:
        ...
        print(f"   Message: {last_msg['text'][:200]}{'...' if len(last_msg['text']) > 200 else ''}")
    else:
