...
# Determine absolute paths for credentials to work from any CWD
...
def gdrive_list(page_size: int = 10, folder_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        output = []
        for item in items:
            output.append(f"ID: {item['id']} | Name: {item['name']} | Type: {item['mimeType']} | Link: {item['webViewLink']}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in cli_commands:
        ...
        parser = argparse.ArgumentParser(description="Google Drive MCP Server & CLI Tool")
