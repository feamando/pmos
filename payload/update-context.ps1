...
if ($TimeFrame -eq "24h") {
    ...
    # User said: "1a. This creates the file with the YYYY-MM-DD-context.md"
    ...
    if ($null -eq $LatestContextFile -and -not (Test-Path (Join-Path $ContextDir "$DateStr-context.md"))) {
        Write-Warning "No context file found for today ($DateStr)."
        Write-Warning "Please run './boot.ps1' to initialize today's context."
        exit 0
    }
    
    # User said: "2a. This creates a new new file YYYY-MM-DD-NN-context.md"
    $RunStr = $NextRun.ToString("00")
    $TargetContextFile = Join-Path $ContextDir "$DateStr-$RunStr-context.md"
}

# Brain Inbox Path (Phase 3 Update)
...
if ($TimeFrame -eq "24h") {
    ...
    # For Jira, we default to -24h if no specific tracking, or -12h for intra-day
    $JiraTime = "-12h" 
}
...
# B. Jira
Write-Host "2. Fetching Jira Updates (Projects: PA, GOC, TPT, RTEVMS, MIO, NVS)..." -ForegroundColor Yellow
...
if (Test-Path $JiraConfig) {
    try {
        ...
        $JQL = "project in (PA, GOC, TPT, RTEVMS, MIO, NVS) AND updated >= $JiraTime ORDER BY updated DESC"
...
if (Test-Path $RawDataFile) {
    # Upload to GDrive
    # Write-Host "Uploading Raw Data to GDrive..." -ForegroundColor Yellow
    # try {
