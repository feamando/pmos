...
### Step 4: Generate External Learnings (v1.3)

After generating the internal learnings, create an external-facing version:
...
**Example (from internal learnings above):**
```
- Identified need for proactive security hardening, resulting in auth guard review added to module checklist
- Learned data hygiene debt accumulates during rapid iteration, resulting in cleanup scripts built alongside features
- Identified experiment monitoring gaps, resulting in weekly health review cadence established
```

### Step 5: Save Learnings to CSV

After generating both internal and external learnings, update the CSV file using this Python script:

```python
import csv
import sys

def update_learnings(csv_path: str, squad_name: str, learnings_text: str, learnings_external: str = ""):
    """Update the Key Learnings and Learnings External columns for a specific squad."""
    rows = []
    fieldnames = None
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            if row['Squad'] == squad_name:
                ...
                # v1.3: Also update external learnings if column exists
                if 'Learnings External' in row and learnings_external:
                    row['Learnings External'] = learnings_external
            rows.append(row)
...
# Usage: update_learnings(csv_path, squad_name, learnings_text, learnings_external)
```

Run this script with:
- `csv_path`: The path to the sprint report CSV
- `squad_name`: The squad name (e.g., "Meal Kit")
- `learnings_text`: The full internal learnings text (everything from "**Learning:**" to the end of "**Sprint Health Signals:**")
- `learnings_external`: The external learnings bullets (v1.3)
...
```
Updated [CSV filename] - [Squad Name] learnings saved (internal + external).
```
