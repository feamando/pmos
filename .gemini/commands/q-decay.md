...
For scheduled monitoring, use the evidence decay monitor tool:

```bash
# Check all evidence
python3 "$PM_OS_COMMON/tools/quint/evidence_decay_monitor.py"

# Generate markdown report
python3 "$PM_OS_COMMON/tools/quint/evidence_decay_monitor.py" --report

# JSON output for automation
python3 "$PM_OS_COMMON/tools/quint/evidence_decay_monitor.py" --json

# Cron-compatible (exit codes: 0=OK, 1=Critical, 2=Expired)
python3 "$PM_OS_COMMON/tools/quint/evidence_decay_monitor.py" --cron --days 14
```

This tool scans:
- `Brain/Reasoning/Evidence/` for standalone evidence files
- `Brain/Reasoning/Decisions/` for DRR-embedded evidence tables
- `.quint/evidence/` for Quint-native evidence
