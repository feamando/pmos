"""
PM-OS CCE Context Iteration Pipeline (v5.0)

Orchestrates context document iteration workflow: v1 generation ->
orthogonal challenge -> v2 revision -> v3 finalization. Tracks versions,
auto-advances on score thresholds, stores all versions for audit trail.

Score Thresholds (PRD F.2):
    v1 -> v2: Score >= 60
    v2 -> v3: Score >= 75
    v3 complete: Score >= 85

Usage:
    from pm_os_cce.tools.feature.context_iteration_pipeline import ContextIterationPipeline
"""
...
# Score thresholds
THRESHOLD_V1_TO_V2 = 60
THRESHOLD_V2_TO_V3 = 75
THRESHOLD_V3_COMPLETE = 85
...
class ContextIterationPipeline:
    ...
    def __init__(self):
        ...
        try:
            ...
            from documents.context_doc_generator import ContextDocGenerator

        try:
            from pm_os_cce.tools.research.orthogonal_integration import OrthogonalIntegration
        except ImportError:
            from research.orthogonal_integration import OrthogonalIntegration
    ...
    def start_pipeline(
        ...
        if auto_challenge:
            ...
            if challenge_result.success:
                ...
                self._extract_decisions_from_challenge(
                    challenge_result, feature_path, version=1
                )
            else:
                next_steps = [
    ...
    def iterate(
        ...
        if next_version == 2:
            unresolved = self._check_unresolved_critical_decisions(feature_path)
            if unresolved:
                critical_texts = [
                    d.get("decision", "unknown")[:80] for d in unresolved
                ]
                return PipelineResult(
                    success=False,
                    message=(
                        f"Cannot generate v2: {len(unresolved)} unresolved critical "
                        f"decision(s) from v1 challenge must be resolved first."
                    ),
                    version=current_version,
                    status=PipelineStatus.PENDING_REVIEW,
                    next_steps=[
                        "Resolve critical decisions before iterating to v2:",
                        *[f"  - {t}" for t in critical_texts[:5]],
                    ],
                    pipeline_state=pipeline_state,
                )
        ...
        if auto_challenge:
            ...
            if challenge_result.success:
                ...
                self._extract_decisions_from_challenge(
                    challenge_result, feature_path, version=next_version
                )
            else:
                next_steps = [
...
"""
    ...
    def _extract_decisions_from_challenge(
        self,
        challenge_result: Any,
        feature_path: Path,
        version: int,
    ) -> List[Dict[str, Any]]:
        """Extract open decisions from orthogonal challenge issues."""
        try:
            from pm_os_cce.tools.feature.feature_state import FeatureState
        except ImportError:
            from feature.feature_state import FeatureState

        if not challenge_result.success or not challenge_result.issues:
            return []
        ...
        for i, issue in enumerate(challenge_result.issues, 1):
            decision_id = f"v{version}-challenge-{i}"
            ...
            state.add_decision(
                decision=issue.text,
                rationale=issue.suggestion or f"Identified by orthogonal challenge v{version}",
                decided_by="orthogonal_challenge",
                metadata=meta,
            )
            decisions_created.append(meta)

        if decisions_created:
            state.save(feature_path)
            logger.info(
                f"Extracted {len(decisions_created)} decisions from v{version} challenge"
            )

        return decisions_created

    def _check_unresolved_critical_decisions(
        self, feature_path: Path
    ) -> List[Dict[str, Any]]:
        """Check for unresolved critical decisions that block v2 generation."""
        try:
            from pm_os_cce.tools.feature.feature_state import FeatureState
        except ImportError:
            from feature.feature_state import FeatureState
        ...
        for d in state.decisions:
            meta = d.metadata
            if (
                meta.get("source", "").endswith("_challenge")
                and meta.get("severity") == "critical"
                and meta.get("status") == "open"
            ):
                unresolved.append(d.to_dict())
    ...
    def archive_and_restart(
        ...
        if state:
            ...
            state.add_decision(
                decision="Archived context documents and restarted pipeline",
                rationale=reason,
                decided_by="user",
            )
