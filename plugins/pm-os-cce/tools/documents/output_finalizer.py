"""
PM-OS CCE OutputFinalizer (v5.0)

Updates all context files with final artifact links after output generation:
1. Updates context.md with PRD link and Jira epic link
...
Usage:
    from pm_os_cce.tools.documents.output_finalizer import OutputFinalizer
"""
...
def _load_feature_state(feature_path: Path):
    """Load FeatureState with v5 import pattern."""
    try:
        ...
        try:
            ...
            return None
    return FeatureState.load(feature_path)
...
class OutputFinalizer:
    ...
    def finalize_context_file(
        ...
        This is the main entry point called after /feature outputs completes.
        """
        result = FinalizationResult(success=False)

        state = _load_feature_state(feature_path)
        if not state:
    ...
    def add_prd_reference(
        self, feature_path: Path, prd_path: Path
    ) -> FinalizationResult:
    ...
    def _update_context_file_links(
        ...
        internal_refs.append(
            "- **Feature State**: [`feature-state.yaml`](feature-state.yaml)"
        )
    ...
    def _update_business_case_links(
        ...
        if "## Related Documents" not in content:
            ...
            related_docs_lines.append("- **Feature State**: `../feature-state.yaml`")

            adr_dir = feature_path / "engineering" / "adrs"
            if adr_dir.exists() and list(adr_dir.glob("adr-*.md")):
                related_docs_lines.append(
                    "- **Architecture Decisions**: `../engineering/adrs/`"
                )
        ...
        if BusinessCaseTrack is not None:
            try:
                bc_track = BusinessCaseTrack(feature_path)
                if bc_track.approvals and "## Approval Records" not in content:
                    approval_lines = ["\n\n## Approval Records\n"]
    ...
    def _update_adr_implementation_status(
        ...
        for adr_file in adr_dir.glob("adr-*.md"):
            ...
            if "## Related Documents" not in content:
                ...
                related_docs_lines.append(
                    f"- **Feature Folder**: `products/{state.organization}/{state.product_id}/{state.slug}/`"
                )
                content += "\n".join(related_docs_lines) + "\n"
                modified = True
    ...
    def _add_prd_back_references(
        self, prd_path: Path, feature_path: Path, state: Any
    ) -> List[str]:
        """Add back-references from PRD to source documents."""
        ...
        try:
            ...
            relative_feature = (
                f"products/{state.organization}/{state.product_id}/{state.slug}"
            )
    ...
    def ensure_bidirectional_links(
        self, feature_path: Path, prd_path: Optional[Path] = None
    ) -> FinalizationResult:
    ...
    def _add_prd_reference(self, feature_path, state, prd_path):
    ...
    def _add_jira_epic_reference(self, feature_path, state, jira_url):
    ...
    def _add_spec_export_reference(self, feature_path, state, spec_path):
        """Add spec export reference to context file."""
        return self._add_artifact_to_references(
            feature_path, state, "Spec Machine Export",
            str(self._get_relative_path(spec_path, feature_path)),
            is_path=True,
        )

    def _add_artifact_to_references(
        self, feature_path, state, artifact_name, artifact_url,
        link_text=None, is_path=False,
    ) -> List[str]:
...
def add_prd_to_context(
    feature_path: Path, prd_path: Path
) -> FinalizationResult:
...
def update_all_artifact_links(
    feature_path: Path, artifacts: Dict[str, str]
) -> FinalizationResult:
...
def verify_bidirectional_links(
    feature_path: Path, prd_path: Optional[Path] = None
) -> FinalizationResult:
    """Verify and fix all bidirectional links between feature artifacts."""
    finalizer = OutputFinalizer()
