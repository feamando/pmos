...
# ============================================================================
# CONFIGURATION — config-driven, no hardcoded paths
# ============================================================================
...
DOCUMENT_BOUNDARIES = [
    r"^# (?:GOOGLE DOCS|GMAIL|JIRA|SLACK|GITHUB)",
    r"^## \d{4}-\d{2}-\d{2}",
    r"^# Daily Context:",
    r"^## Critical",
    r"^## Key Decisions",
...
def _get_state_dir() -> Path:
    """Get chunker state directory from environment."""
    user_dir = os.environ.get("PM_OS_USER", "")
    if user_dir:
        return Path(user_dir) / ".cache" / "chunker_state"

    # Walk up from script
    current = Path(__file__).resolve().parent
    for _ in range(10):
        candidate = current / "user" / ".cache" / "chunker_state"
        if candidate.parent.parent.exists():
            return candidate
        current = current.parent

    return Path.home() / "pm-os" / "user" / ".cache" / "chunker_state"
...
class Chunk:
    ...
    def to_dict(self) -> dict:
        d = asdict(self)
        d["content"] = f"[{len(self.content)} chars]"
        return d
...
def print_scan_results(results: List[FileInfo], threshold: int):
    ...
    for info in results[:10]:
        marker = "! " if info.needs_chunking else "  "
        print(f"  {marker}{info.lines:>6,} lines | {info.path}")
...
def main():
    ...
    parser.add_argument("--check", type=str, metavar="FILE", help="Check if a file needs chunking")
    parser.add_argument("--split", type=str, metavar="FILE", help="Split a file into chunks")
    parser.add_argument("--scan", type=str, metavar="DIR", help="Scan directory for large files")
    ...
    parser.add_argument(
        "--threshold", "-t", type=int, default=DEFAULT_MAX_LINES,
        help=f"Line threshold (default: {DEFAULT_MAX_LINES})",
    )
    parser.add_argument(
        "--overlap", type=int, default=OVERLAP_LINES,
        help=f"Overlap lines (default: {OVERLAP_LINES})",
    )
