...
Captures session context before compaction to preserve objectives,
decisions, files touched, and open questions.

Usage:
    python3 session_manager.py --create "Title"
    python3 session_manager.py --save --tags "v5,phase-1"
    python3 session_manager.py --load [session_id]
    python3 session_manager.py --search "query"
    python3 session_manager.py --status
    python3 session_manager.py --list [n]
"""
...
# Path resolution — config-driven, not hardcoded
_SESSIONS_DIR: Optional[Path] = None


def _get_sessions_dir() -> Path:
    """Resolve sessions directory from environment or relative paths."""
    ...
    if user_dir:
        _SESSIONS_DIR = Path(user_dir) / "sessions"
    else:
        # Walk up from script location
        current = Path(__file__).resolve().parent
        for _ in range(10):
            candidate = current / "user" / "sessions"
            if candidate.exists():
                _SESSIONS_DIR = candidate
                break
            current = current.parent
        if not _SESSIONS_DIR:
            _SESSIONS_DIR = Path.home() / "pm-os" / "user" / "sessions"

    return _SESSIONS_DIR


def _active_dir() -> Path:
    return _get_sessions_dir() / "Active"


def _archive_dir() -> Path:
    return _get_sessions_dir() / "Archive"


def _index_file() -> Path:
    return _get_sessions_dir() / "Index.md"


def _current_session_path() -> Path:
    return _active_dir() / "current.md"
...
def generate_session_id() -> str:
    ...
    existing = list(_archive_dir().glob(f"{date_str}-*.md"))
    existing += list(_active_dir().glob(f"{date_str}-*.md"))
...
class SessionManager:
    ...
    def __init__(self):
        ensure_directories()
        self.current_session_path = _current_session_path()
    ...
    def get_current_session(self) -> Optional[Dict[str, Any]]:
        ...
        if self.current_session_path.exists():
            ...
            return {"frontmatter": frontmatter, "body": body, "path": self.current_session_path}
        return None

    def create_session(self, title: str, objectives: List[str] = None, tags: List[str] = None) -> str:
        ...
        frontmatter = {
            ...
            "started": now, "last_updated": now, "status": "active",
            "tags": tags or [], "files_created": [], "files_modified": [],
            "files_explored": [], "decisions": [],
        }

        objectives_md = "\n".join([f"- [ ] {obj}" for obj in (objectives or ["Define objectives"])])
        body = f"""
# Session: {title}
...
"""
    ...
    def update_session(self, updates: Dict[str, Any]) -> bool:
        ...
        for field in ["tags", "files_created", "files_modified", "files_explored", "decisions"]:
            if field in updates:
    ...
    def add_decision(self, decision: str, rationale: str, alternatives: str = "") -> bool:
        ...
        decision_record = {
            ...
            "alternatives": alternatives, "timestamp": datetime.now().isoformat(),
        }
    ...
    def archive_session(self, summary: str = None) -> Optional[str]:
        ...
        started = datetime.fromisoformat(frontmatter.get("started", datetime.now().isoformat()))
        ...
        archive_path = _archive_dir() / f"{session_id}.md"
...
"""
        ...
        if idx.exists():
            ...
            idx.write_text(current)
        else:
            idx.write_text(f"# Session Index\n\n## Recent Sessions\n{index_entry}\n")
    ...
    def load_session(self, session_id: str = None) -> Optional[Dict[str, Any]]:
        ...
        if session_id:
            ...
            matches = list(_archive_dir().glob(f"*{session_id}*.md"))
            if matches:
                content = matches[0].read_text()
                frontmatter, body = parse_frontmatter(content)
                return {"frontmatter": frontmatter, "body": body, "path": matches[0]}
        else:
            sessions = sorted(_archive_dir().glob("*.md"), reverse=True)
            if sessions:
    ...
    def search_sessions(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for session_file in _archive_dir().glob("*.md"):
            ...
            if tag_match or title_match or body_match:
                score = (3 if tag_match else 0) + (2 if title_match else 0) + (1 if body_match else 0)
                results.append({
                    "session_id": frontmatter.get("session_id"),
                    "title": title, "date": frontmatter.get("started", "")[:10],
                    "tags": tags, "score": score, "path": session_file,
                })
    ...
    def list_sessions(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        if self.current_session_path.exists():
            ...
            sessions.append({
                ...
                "status": "active", "tags": frontmatter.get("tags", []),
            })

        for session_file in sorted(_archive_dir().glob("*.md"), reverse=True)[:limit]:
            content = session_file.read_text()
            frontmatter, _ = parse_frontmatter(content)
            sessions.append({
                "session_id": frontmatter.get("session_id"),
                ...
                "date": frontmatter.get("started", "")[:10],
                "status": "archived", "tags": frontmatter.get("tags", []),
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "status": "active", "session_id": fm.get("session_id"),
            "title": fm.get("title"), "started": fm.get("started"),
    ...
    def generate_summary(self) -> str:
        ...
        if objectives:
            ...
            for checked, obj in objectives[:5]:
...
def main():
    ...
    parser.add_argument("--create", "-c", type=str, help="Create new session with title")
    parser.add_argument("--save", "-s", action="store_true", help="Save/update current session")
    parser.add_argument("--load", "-l", type=str, nargs="?", const="", help="Load session by ID")
    parser.add_argument("--search", type=str, help="Search sessions by query")
    parser.add_argument("--archive", "-a", action="store_true", help="Archive current session")
    parser.add_argument("--status", action="store_true", help="Show session status")
    parser.add_argument("--list", "-n", type=int, nargs="?", const=10, help="List recent sessions")
    ...
    parser.add_argument("--decision", type=str, help="Add decision (format: decision|rationale|alternatives)")
    ...
    parser.add_argument("--files-created", type=str, help="Files created (comma-separated)")
    parser.add_argument("--files-modified", type=str, help="Files modified (comma-separated)")
    ...
    if args.create:
        ...
        if args.files_created:
            updates["files_created"] = [f.strip() for f in args.files_created.split(",")]
        if args.files_modified:
            updates["files_modified"] = [f.strip() for f in args.files_modified.split(",")]
        if updates:
            manager.update_session(updates)
        else:
            status = manager.get_status()
            if status["status"] == "active":
                print(f"Session active: {status['session_id']} - {status['title']}")
            else:
        ...
        if status["status"] == "active":
            ...
            print(f"Files: {status['files_created']} created, {status['files_modified']} modified")
