...
try:
    from pm_os_base.tools.core.config_loader import get_config, get_team_config
except ImportError:
    try:
        from core.config_loader import get_config, get_team_config
    except ImportError:
        get_config = None
        get_team_config = None
...
class BrainIndexGenerator:
    ...
    def _load_tier1_entities(self) -> List[Dict[str, Any]]:
        ...
        All values come from config — zero hardcoded names/orgs.
        """
        team_config = {}
        user_config = {}
        if get_team_config is not None:
            try:
        ...
        if get_config is not None:
            try:
                user_config = get_config().get("user", {}) or {}
            except Exception:
                pass
        ...
        user_id = user_name.lower().replace(" ", "-").replace("_", "-") if user_name else ""
        if user_id:
            members.append({
                "id": user_id,
                "name": user_name,
    ...
    def _get_hot_topic_ids(self) -> List[str]:
        ...
        try:
            try:
                from pm_os_brain.tools.brain_core.brain_loader import (
                    build_alias_index,
                    load_registry,
                    scan_for_entities,
                ...
                from brain_core.brain_loader import (
                    build_alias_index,
                    load_registry,
                    scan_for_entities,
                    get_latest_context_file,
