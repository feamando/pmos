...
Usage:
    ...
    python brain_query.py "some query" --no-graph
...
# Sibling imports
try:
    from pm_os_brain.tools.index.brain_graph import BrainGraph
except ImportError:
    try:
        from index.brain_graph import BrainGraph
    except ImportError:
        BrainGraph = None

try:
    from pm_os_brain.tools.index.brain_search import BrainSearch, SearchResult
except ImportError:
    try:
        from index.brain_search import BrainSearch, SearchResult
    except ImportError:
        BrainSearch = None
        SearchResult = None
...
class BrainQuery:
    ...
    def __init__(self, brain_path: Optional[Path] = None):
        ...
        self._graph = None  # Optional[BrainGraph]
    ...
    def query(
        ...
        if use_graph and seeds and BrainGraph is not None:
    ...
    def _merge_and_rank(self, seeds, neighbors) -> List:
        """
    ...
    def get_entity_context(
        ...
        if include_neighbors and SearchResult is not None:
...
Examples:
  ...
  python brain_query.py "person name" --limit 5 --decay 0.3
    ...
    parser.add_argument(
        "--no-graph",
        action="store_true",
        help="Disable graph expansion (BRAIN only)",
    )
    parser.add_argument(
        "--decay",
        "-d",
