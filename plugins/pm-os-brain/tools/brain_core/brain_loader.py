#!/usr/bin/env python3
"""
Brain Loader - Hot Topics Identification (v5.0)
...
_paths = get_paths()
BRAIN_DIR = str(_paths.brain)
REGISTRY_FILE = os.path.join(BRAIN_DIR, "registry.yaml")
CONTEXT_DIR = str(_paths.context)
QUINT_DIR = str(_paths.root / ".quint")
...
def load_registry() -> Dict:
    ...
    if not os.path.exists(REGISTRY_FILE):
        # Auto-create empty v2 registry on first boot
        ...
        with open(REGISTRY_FILE, "w", encoding="utf-8") as wf:
            if HAS_YAML:
                yaml.dump(empty_registry, wf, default_flow_style=False)
            else:
                json.dump(empty_registry, wf)
        logger.info("Created empty brain registry at %s", REGISTRY_FILE)
        return {}
    ...
    with open(REGISTRY_FILE, "r", encoding="utf-8") as f:
        if HAS_YAML:
            ...
            logger.warning("Cannot parse registry without PyYAML")
            return {}
...
def format_reasoning_output(state: Dict) -> str:
    ...
    if state["expiring_evidence"]:
        ...
        lines.append("")
        lines.append("  Run `/reason decay` to refresh or waive.")
        lines.append("")
    ...
    if state["active_cycles"] > 0:
        ...
        lines.append("  Run `/reason status` for details")
    else:
        lines.append("STATUS: No active reasoning cycles")
        lines.append("  Run `/reason hypothesize <problem>` to start")
...
def main():
    ...
    if args.query:
        ...
        logger.info("Scanning: %s", os.path.basename(context_file))
    ...
    if not args.query:
        ...
        try:
            ...
            logger.warning("Could not save hot_topics.json: %s", e)
