#!/usr/bin/env python3
"""
Brain Updater - Changelog Append Tool (v5.0)
...
def update_section(
    ...
    if dry_run:
        logger.info("[DRY-RUN] Would update section '%s' in %s", section_header, file_path)
        return True
...
def _append_v2_event(
    ...
    try:
        ...
        try:
            ...
            logger.warning("EventHelper not available, skipping v2 event append")
            return False
    ...
    if dry_run:
        logger.info("[DRY-RUN] Would append v2 event to %s: %s", full_path, message)
        return True
...
def append_changelog_entry(
    ...
    if dry_run:
        logger.info("[DRY-RUN] Would append to %s: %s", file_path, entry.strip())
        return True
...
def main():
    ...
    if not registry:
        logger.error("Could not load registry")
        sys.exit(1)
    ...
    if not significant_matches:
        logger.info("No significant entity mentions found.")
        return
    ...
    for entity_id, data in sorted(
        ...
        if success:
            ...
            print("  [SKIP] Entry already exists or file missing")
            skipped_count += 1
