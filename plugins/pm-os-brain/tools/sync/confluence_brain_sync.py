...
def _get_confluence_config() -> Dict[str, Any]:
    """Get Confluence configuration from config + env."""
    config = get_config()
    confluence_cfg = config.get("integrations.confluence", {}) or {}

    url = confluence_cfg.get("url") or os.getenv("CONFLUENCE_URL", "")
    email = confluence_cfg.get("email") or os.getenv("CONFLUENCE_EMAIL", "")
    api_token = os.getenv("CONFLUENCE_API_TOKEN", "")
    spaces = confluence_cfg.get("spaces", [])

    if not spaces:
        spaces_env = os.getenv("CONFLUENCE_SPACES", "")
        if spaces_env:
            spaces = [s.strip() for s in spaces_env.split(",") if s.strip()]

    # Try connector_bridge for token
    if get_auth is not None and not api_token:
...
def get_confluence_client():
    ...
    if not conf["url"] or not conf["email"] or not conf["api_token"]:
        logger.error(
            "Confluence credentials not configured. Set CONFLUENCE_URL, CONFLUENCE_EMAIL, "
            "CONFLUENCE_API_TOKEN in .env or configure integrations.confluence in config.yaml"
        )
        return None
            ...
            pages.append(
                {
                    ...
                    "url": f"{conf['url']}/wiki{content.get('_links', {}).get('webui', '')}",
...
def fetch_page_content(page_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return {
            ...
            "url": f"{conf['url']}/wiki{page.get('_links', {}).get('webui', '')}",
