...
Usage:
    ...
    paths = get_paths()
    store = EventStore(paths.user / "brain")
"""
...
class EventStore:
    ...
    def append_event(
        ...
        # Generate event ID
        event_id = f"evt-{now.strftime('%Y%m%d-%H%M%S')}"
    ...
    def _load_entity_events(self, entity_path: Path) -> List[Event]:
        ...
        try:
            ...
            logger.debug("Failed to load events from %s: %s", entity_path, exc)
    ...
    def _write_event_to_entity(self, entity_path: Path, event: Event) -> bool:
        ...
        try:
            ...
            frontmatter["$updated"] = datetime.now(timezone.utc).isoformat() + "Z"
            ...
            logger.debug("Failed to write event to %s: %s", entity_path, exc)
            return False
