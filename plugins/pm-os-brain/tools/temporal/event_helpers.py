#!/usr/bin/env python3
"""
PM-OS Brain Event Helpers (v5.0)
...
try:
    ...
    try:
        from pm_os_base.tools.core.entity_validator import ChangeEvent, EventType, FieldChange
    except ImportError:
        # Minimal fallback — define the enums inline so the module still works
        # when schemas package is not on the path
        import enum
...
class EventHelper:
    ...
    def create_event(
        ...
        Args:
            ...
            actor: Who/what made the change (e.g., "system/relationship_builder")
        ...
        # If Pydantic models are available, validate through them
        if ChangeEvent is not None and FieldChange is not None:
            ...
            # Fallback: build dict manually without Pydantic validation
            data = {
                "event_id": event_id,
            ...
            for change in (changes or []):
                ...
                if change.get("value") is not None:
                    change_dict["value"] = change["value"]
                if change.get("old_value") is not None:
                    change_dict["old_value"] = change["old_value"]
                data["changes"].append(change_dict)
