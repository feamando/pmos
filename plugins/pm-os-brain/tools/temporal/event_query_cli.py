#!/usr/bin/env python3
"""
PM-OS Brain Event Query CLI (v5.0)
...
Usage:
    python3 event_query_cli.py timeline Entities/People/Some_Person.md
...
def resolve_brain_path() -> Path:
    """Resolve the brain directory path via config."""
    try:
    ...
    try:
        ...
        return Path(__file__).resolve().parent.parent.parent.parent.parent / "user" / "brain"
...
def cmd_timeline(store: EventStore, brain_path: Path, entity_ref: str) -> int:
    ...
    if not entity_path.exists():
        ...
        if not entity_path.exists():
            logger.error("Entity not found: %s", entity_ref)
