#!/usr/bin/env python3
"""
PM-OS Brain Registry V2 Builder (v5.0)
...
class RegistryV2Builder:
    ...
    def build(self, incremental: bool = False) -> Dict[str, Any]:
        ...
        # Update timestamp
        registry["$generated"] = datetime.now(timezone.utc).isoformat() + "Z"

        logger.info("Scanned %d entities", entities_scanned)
    ...
    def _extract_v1_entry(
        ...
        return {
            ...
            "$status": frontmatter.get("status", "active"),
            "$version": 1,
            "$updated": frontmatter.get("updated", datetime.now(timezone.utc).isoformat() + "Z"),
            "aliases": frontmatter.get("aliases", []),
            "role": frontmatter.get("role"),
            "team": frontmatter.get("team"),
            "owner": frontmatter.get("owner"),
