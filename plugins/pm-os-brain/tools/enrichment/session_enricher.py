#!/usr/bin/env python3
"""
Session Enricher (v5.0)

Enriches Brain entities from Claude Code session research findings.
Processes research findings captured during Claude Code sessions
and updates relevant Brain entities with discovered knowledge.

Local-only enricher (no external API calls).
"""
...
class SessionEnricher(BaseEnricher):
    ...
    def enrich(self, item: Dict[str, Any], dry_run: bool = False) -> int:
        ...
        for entity_ref in related_entities:
            ...
            # Check for duplicate event before appending
            if self.has_existing_event(entity_data, finding_id, session_id):
                logger.debug(
                    "Skipping duplicate event: finding=%s session=%s entity=%s",
                    finding_id, session_id, canonical,
                )
                continue
...
def main():
    ...
    # Resolve paths via path_resolver
    brain_path = None
    if args.brain:
