...
Called after unified_brain_writer, before synapse_builder.

Config-driven enrichment schedules and thresholds.
...
try:
    ...
    from ..relationships.embedding_edge_inferrer import EmbeddingEdgeInferrer
    from .gdocs_enricher import GDocsEnricher
    from .github_enricher import GitHubEnricher
    from .jira_enricher import JiraEnricher
    from ..quality.graph_health import GraphHealthMonitor
except ImportError:
    try:
...
class BrainEnrichmentOrchestrator:
    ...
    def _get_enrichment_config(self) -> Dict[str, Any]:
        """Get enrichment configuration from config.yaml."""
        if get_config is None:
            return {}
        try:
            config = get_config()
            return config.get("brain.enrichment", {}) or {}
        except Exception:
            return {}
    ...
    def run(
        ...
        enrichment_config = self._get_enrichment_config()
        if enrichment_config:
            limit = enrichment_config.get("orphan_limit", limit)
    ...
    def _run_body_extraction(self, dry_run: bool, limit: int) -> Dict[str, int]:
        ...
        This is the most effective enrichment method.
        """
        if BodyRelationshipExtractor is None:
            return {"relationships_created": 0, "error": "BodyRelationshipExtractor not available"}
    ...
    def _run_external_enrichment(
        ...
        Handles auth gracefully via connector_bridge.
        """
        results = {}

        # GDocs enrichment
        if GDocsEnricher is not None:
            try:
    ...
    def _run_embedding_edges(self, dry_run: bool, limit: int) -> Dict[str, int]:
        ...
        if EmbeddingEdgeInferrer is None:
            return {"relationships_created": 0, "warning": "EmbeddingEdgeInferrer not available"}
...
def main():
    ...
    parser.add_argument("--entity-types", type=str, help="Comma-separated entity types to process")
