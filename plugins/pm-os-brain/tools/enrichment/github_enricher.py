#!/usr/bin/env python3
"""
GitHub Enricher (v5.0)

Enriches Brain entities from GitHub commits, PRs, and issues.
Uses connector_bridge for GitHub authentication (three-tier auth).
Config-driven: repos loaded from config.yaml, no hardcoded org names.

Enhanced for orphan cleanup:
- enrich_orphans(): Targets orphan system entities
...
class GitHubEnricher(BaseEnricher):
    ...
    - env: Direct API via GITHUB_TOKEN from .env
    - none: Not configured
    """
    ...
    def _get_github_config(self) -> Dict[str, Any]:
        """Get GitHub configuration from config.yaml."""
        if get_config is None:
            return {}
        try:
            config = get_config()
            return config.get("integrations.github", {}) or {}
        except Exception:
            return {}

    def _get_default_repos(self) -> List[str]:
        """Get default repos from config instead of hardcoded values."""
        github_config = self._get_github_config()
        repos = github_config.get("repos", [])
        if repos:
            return repos if isinstance(repos, list) else [repos]
        # Fall back to org + repo pattern from config
        org = github_config.get("org", "")
        default_repo = github_config.get("default_repo", "")
        if org and default_repo:
            return [f"{org}/{default_repo}"]
        return []
    ...
    def _enrich_from_issue(self, item: Dict[str, Any], dry_run: bool) -> int:
        ...
        for entity_slug in mentioned_entities:
            ...
            if updates:
                corr_id = f"github-issue-{issue_number}"
                if self.has_existing_event_by_correlation(frontmatter, corr_id):
                    continue
    ...
    def enrich_orphans(
        ...
        Uses connector_bridge for auth and config for repo list.

        Args:
            repos: List of repos to search (default: from config)
        ...
        # Get repos from config if not explicitly provided
        target_repos = repos or self._get_default_repos()
        if not target_repos:
            logger.warning("No GitHub repos configured. Set integrations.github.repos in config.yaml")
            return {
                "total_orphans": 0, "processed": 0, "enriched": 0,
                "no_github_data": 0, "relationships_created": 0,
                "api_calls": 0, "details": [],
            }

        # Check auth
        if get_auth is not None:
            auth = get_auth("github")
            if auth.source == "none":
                logger.warning("GitHub not configured: %s", auth.help_message)
        ...
        for orphan_path, frontmatter, body in orphans:
            ...
            if github_matches:
                rels_created = self._create_relationships_from_github(
                    entity_id, entity_type, github_matches, builder, dry_run,
                )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Repos to search (comma-separated, default: from config)",
    )
    parser.add_argument(
        "--types", type=str, default="system,project",
    ...
    logger.info("Enriching orphans from GitHub...")
    results = enricher.enrich_orphans(
        repos=repos, entity_types=entity_types,
    ...
    print(f"No GitHub data found: {results.get('no_github_data', 0)}")
