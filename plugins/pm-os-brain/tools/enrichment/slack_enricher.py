...
Uses connector_bridge for Slack authentication.
"""
...
class SlackEnricher(BaseEnricher):
    ...
    - env: Direct API via SLACK_BOT_TOKEN from .env
    - none: Not configured
    """
    ...
    def _check_auth(self) -> Optional[object]:
        """Check Slack auth availability via connector_bridge."""
        if get_auth is None:
            logger.debug("connector_bridge not available, using offline mode")
            return None
        auth = get_auth("slack")
        if auth.source == "none":
            logger.warning("Slack auth not configured: %s", auth.help_message)
            return None
        return auth
    ...
    def enrich(self, item: Dict[str, Any], dry_run: bool = False) -> int:
        ...
        for entity_slug in mentioned_entities:
            ...
            if updates:
                ...
                corr_id = f"slack-{thread_ts or timestamp}"
