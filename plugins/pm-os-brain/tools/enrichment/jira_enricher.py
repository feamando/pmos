...
Uses connector_bridge for Jira authentication (three-tier auth).
Config-driven: no hardcoded project prefixes or URLs.

Enhanced for orphan cleanup:
- enrich_orphans(): Targets orphan entities
...
class JiraEnricher(BaseEnricher):
    ...
    - env: Direct API via JIRA_API_TOKEN from .env
    - none: Not configured
    """
    ...
    def _get_jira_config(self) -> Dict[str, Any]:
        """Get Jira configuration from config.yaml."""
        if get_config is None:
            return {}
        try:
            config = get_config()
            return config.get("integrations.jira", {}) or {}
        except Exception:
            return {}
    ...
    def enrich_orphans_live(
        ...
        Uses connector_bridge for authentication:
        - connector: Data comes from Claude connector (pass as input)
        - env: Direct API call using JIRA_API_TOKEN from .env
        - none: Fall back to cached data
        ...
        if get_auth is not None:
            ...
            if auth.source == "connector":
                # Data comes from Claude connector - pass as input
                logger.info("Jira connector available, but live enrichment needs cached data")
                return self.enrich_orphans(
                    entity_types=entity_types, limit=limit, dry_run=dry_run
                )
            elif auth.source == "env":
                # Direct API call using token from .env
                return self._enrich_orphans_via_api(
                    auth.token, entity_types, limit, dry_run
                )
            elif auth.source == "none":
                logger.warning("Jira not configured: %s", auth.help_message)
                return self.enrich_orphans(
                    entity_types=entity_types, limit=limit, dry_run=dry_run
                )

        # Fallback: try cache mode
        return self.enrich_orphans(
            entity_types=entity_types, limit=limit, dry_run=dry_run
        )

    def _enrich_orphans_via_api(
        self,
        token: str,
        ...
        """Enrich orphans using direct Jira API with token."""
        try:
            from atlassian import Jira
        except ImportError:
            logger.warning("atlassian-python-api not installed, using cache mode")
            return self.enrich_orphans(
                entity_types=entity_types, limit=limit, dry_run=dry_run
            )
        ...
        try:
            jira = Jira(
                url=jira_url, username=jira_user, password=token, cloud=True
            )
...
def main():
    ...
    parser.add_argument("--jira-cache", type=Path, help="Path to Jira data cache")
    parser.add_argument(
        "--types", type=str, default="project,person,system",
    ...
    print(f"No Jira data found: {results.get('no_jira_data', 0)}")
