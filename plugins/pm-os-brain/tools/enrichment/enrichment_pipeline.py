#!/usr/bin/env python3
"""
PM-OS Brain Enrichment Pipeline (v5.0)

Orchestrates re-processing of all data sources to increase entity data density.
Uses connector_bridge for external service auth, path_resolver for paths.
...
class EnrichmentPipeline:
    ...
    def run(
        ...
        if resume and self.checkpoint_file.exists():
            ...
            logger.info(
                "Resuming from checkpoint: %d/%d",
                self.progress.processed_entities,
                self.progress.total_entities,
            )
        else:
        ...
        logger.info("  Total entities: %d", self.progress.total_entities)
        logger.info("  Sources: %s", ", ".join(sources))
        logger.info("  Workers: %d", self.max_workers)
        ...
        for source in sources:
            ...
            if not enricher:
                logger.info("  No enricher available for %s", source)
                continue

            # Get data for this source
            source_data = self._load_source_data(source)
            if not source_data:
                logger.info("  No data found for %s", source)
            ...
            logger.info("  Completed %s: %d entities processed", source, len(results))
    ...
    def _get_enricher(self, source: str):
        ...
        try:
            ...
            logger.warning("Could not import enricher for %s: %s", source, e)
            return None
        return None
    ...
    def _load_source_data(self, source: str) -> List[Dict[str, Any]]:
        ...
        if not data_dir.exists():
            ...
            alt_paths = {
                "gdocs": self.brain_path / "Inbox" / "GDocs" / "Batch",
                "slack": self.brain_path / "Inbox" / "Slack" / "Messages",
                "jira": self.brain_path / "Inbox" / "Jira",
                "github": self.brain_path / "Inbox" / "GitHub",
            }
    ...
    def _load_context_data(self) -> List[Dict[str, Any]]:
        ...
        # Use path_resolver for context directory
        context_dir = None
        ...
        if context_dir is None:
            # Fallback: relative to brain path
    ...
    def _get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "results": [r.__dict__ for r in self.results[:100]],
        }
...
def main():
    ...
    # Default brain path via path_resolver
