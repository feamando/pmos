"""Tests for pm-os-brain plugin structure: manifest, commands, skills, pipelines."""
...
class TestPluginManifest:
    ...
    def test_name(self):
        assert self.manifest["name"] == "pm-os-brain"
    ...
    def test_commands_declared(self):
        ...
        assert len(cmds) >= 1, "Must declare at least one command"

    def test_skills_declared(self):
        skills = self.manifest.get("skills", [])
        assert len(skills) >= 1, "Must declare at least one skill"

    def test_mcp_servers_declared(self):
        mcp = self.manifest.get("mcpServers", {})
        assert "pm-os-brain" in mcp, "Must declare brain MCP server"
...
class TestPreflightChecks:
    ...
    def test_preflight_yaml_valid(self):
        ...
        assert len(data) >= 1, "Must define at least one check category"
...
class TestMcpJson:
    ...
    def test_mcp_json_exists(self):
        ...
        assert "pm-os-brain" in data["mcpServers"]
